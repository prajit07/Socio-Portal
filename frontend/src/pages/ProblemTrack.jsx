...
function Timeline({ status }) {
  ...
  const TIMELINE = [
    { key: 'pending_validation', label: t('Submitted') },
    { key: 'validated', label: t('Validated') },
    { key: 'open', label: t('Open & Routed') },
    { key: 'in_review', label: t('In Review') },
    { key: 'proposal_submitted', label: t('Proposal Submitted') },
    { key: 'in_collaboration', label: t('In Collaboration') },
    { key: 'prototype', label: t('Prototype') },
    { key: 'pilot', label: t('Pilot') },
    { key: 'implemented', label: t('Implemented') },
  ];
...
export default function ProblemTrack() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError(e.response?.data?.detail || t('Failed to load problem.'));
    } finally {
      setLoading(false);
    }
  }, [id]);
  ...
  const runAnalysis = async () => {
    ...
    try {
      ...
      setError(e.response?.data?.detail || t('Analysis failed.'));
    } finally {
      setAnalyzing(false);
    }
  };
  ...
  if (error && !problem) return (
    <div className="min-h-screen bg-bg-soft"><Navbar /><main className="mx-auto max-w-3xl px-4 py-12"><Alert variant="danger">{error}</Alert><Link to="/problems" className="mt-4 inline-block"><Button>{t('Back to Problems')}</Button></Link></main></div>
  );
      ...
      <main className="mx-auto max-w-[1100px] px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-6">
            <Card>
              ...
              {problem.address && (
                <div className="mt-4 flex items-start gap-2 text-sm text-ink-soft bg-bg-soft rounded-card p-3">
                  ...
                  <div><div className="font-semibold text-primary-navy">{t('Location')}</div>{problem.address}{problem.latitude && <div className="text-xs text-ink-muted">{problem.latitude}, {problem.longitude}</div>}</div>
                </div>
              )}

              {problem.ai_tags?.length > 0 && (
                <div className="mt-4">
                  <div className="text-xs font-semibold uppercase tracking-wide text-ink-muted mb-2">{t('AI Tags')}</div>
              ...
              {problem.ai_duplicate_of && (
                <Alert variant="warning" className="mt-4">
                  {t('This problem was flagged as a possible duplicate of')}{' '}
            ...
            {/* Evidence */}
            <Card>
              <h2 className="font-bold text-primary-navy mb-3">{t('Evidence')} ({evidence.length})</h2>
              {evidence.length === 0 ? (
                <p className="text-sm text-ink-muted">{t('No evidence uploaded yet.')}</p>
              ) : (
              ...
              {evidence.find((e) => e.transcript) && (
                <div className="mt-3">
                  <div className="text-xs font-semibold uppercase tracking-wide text-ink-muted mb-1">{t('Transcripts')}</div>
            ...
            <Card>
              <h2 className="font-bold text-primary-navy mb-3">{t('Proposals')} ({solutions.length})</h2>
              {solutions.length === 0 ? (
                <p className="text-sm text-ink-muted">{t('No proposals yet. University teams will draft these in the next phase.')}</p>
              ) : (
                  ...
                  {solutions.map((s) => (
                    <li key={s.id}>
                      <Link to={`/problems/${id}/solutions/${s.id}`} className="block border border-line rounded-card p-3 hover:border-primary hover:bg-primary/5 transition">
                        ...
                        <p className="text-xs text-primary mt-2 font-semibold">{t('View details →')}</p>
                      </Link>
                    </li>
                  ))}
                </ul>
          ...
          <div className="space-y-6">
            <Card>
              <h3 className="font-bold text-primary-navy mb-3">{t('Progress')}</h3>
              ...
              {isOwner && (
                <Button variant="secondary" size="sm" className="mt-4 w-full" onClick={runAnalysis} loading={analyzing}>
                  {t('Re-run AI Analysis')}
                </Button>
              )}
            </Card>
            <Card>
              <h3 className="font-bold text-primary-navy mb-2">{t('Submitted')}</h3>
