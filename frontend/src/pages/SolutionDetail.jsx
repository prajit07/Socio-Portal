...
export default function SolutionDetail() {
  ...
  const roleLabels = {
    citizen: t('Citizen'), student: t('Student'), faculty: t('Faculty'),
    university_admin: t('University Admin'), industry: t('Industry'),
    government: t('Government'), admin: t('Admin'),
  };

  const INDUSTRY_ACTIONS = [
    {
      id: 'interested',
      label: t('Express Interest'),
      desc: t('Signal interest to start a conversation'),
      icon: '🤝',
      color: 'bg-blue-600 hover:bg-blue-700',
    },
    {
      id: 'funding',
      label: t('Fund Proposal'),
      desc: t('Commit funding to this solution'),
      icon: '💰',
    ...
    },
    {
      id: 'prototype',
      label: t('Co-Develop'),
      desc: t('Join as a co-development partner'),
      icon: '🔬',
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      setError(e.response?.data?.detail || t('Failed to load solution.'));
    } finally {
      setLoading(false);
    }
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      alert(e.response?.data?.detail || t('Failed to update.'));
    }
  };

  const handleIndustryAction = async (stage) => {
    if (!industry) {
      setError(t('Please create your Industry profile first (Industry Dashboard → Profile tab).'));
      return;
    }
    setActionBusy(stage);
    setError('');
    try {
      ...
      setError(e.response?.data?.detail || t('Failed to start collaboration.'));
    } finally {
      setActionBusy('');
    }
  };
    ...
    <div className="min-h-screen bg-bg-soft">
      ...
      <main className="max-w-3xl mx-auto px-4 py-12 text-center">
        <Card className="py-12">
          <h2 className="text-xl font-semibold text-primary-navy">{t('Solution Not Found')}</h2>
          <p className="text-ink-soft mt-2">{error}</p>
          <Link to={`/problems/${problemId}`} className="mt-4 inline-block">
            <Button>{t('Back to Problem')}</Button>
          </Link>
        </Card>
      </main>
    </div>
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="p-6 border-b border-line flex justify-between items-start">
                <div>
                  ...
                  <div className="flex gap-2 mt-2 flex-wrap">
                    ...
                    <Badge variant="outline">{roleLabels[solution.author?.role] || t('Unknown')}</Badge>
                  </div>
                </div>
              ...
              <div className="p-6">
                {editing ? (
                  <form onSubmit={handleUpdate} className="space-y-4">
                    <div>
                      <label className="block text-sm font-semibold text-ink mb-1">{t('Title *')}</label>
                    ...
                    <div>
                      <label className="block text-sm font-semibold text-ink mb-1">{t('Description *')}</label>
                    ...
                    <div>
                      <label className="block text-sm font-semibold text-ink mb-1">{t('Approach')}</label>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="block text-sm font-semibold text-ink mb-1">{t('Tech Stack')}</label>
                        <input value={Array.isArray(editData.tech_stack) ? editData.tech_stack.join(', ') : editData.tech_stack || ''} onChange={(e) => setEditData({ ...editData, tech_stack: e.target.value })} className="w-full px-4 py-2.5 rounded-lg border border-line focus:border-primary focus:ring-2 focus:ring-primary/20 text-sm" placeholder={t('React, Python, FastAPI')} />
                      </div>
                      <Select label={t('Status')} value={editData.status} onChange={(e) => setEditData({ ...editData, status: e.target.value })} options={[
                        { value: 'draft', label: t('Draft') }, { value: 'submitted', label: t('Submitted') },
                        { value: 'under_review', label: t('Under Review') }, { value: 'accepted', label: t('Accepted') },
                        { value: 'rejected', label: t('Rejected') },
                      ]} />
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="block text-sm font-semibold text-ink mb-1">{t('Timeline')}</label>
                        <input value={editData.estimated_timeline || ''} onChange={(e) => setEditData({ ...editData, estimated_timeline: e.target.value })} className="w-full px-4 py-2.5 rounded-lg border border-line focus:border-primary focus:ring-2 focus:ring-primary/20 text-sm" placeholder={t('3 months')} />
                      </div>
                      <div>
                        <label className="block text-sm font-semibold text-ink mb-1">{t('Budget')}</label>
                        <input value={editData.estimated_budget || ''} onChange={(e) => setEditData({ ...editData, estimated_budget: e.target.value })} className="w-full px-4 py-2.5 rounded-lg border border-line focus:border-primary focus:ring-2 focus:ring-primary/20 text-sm" placeholder={t('₹5,00,000')} />
                      </div>
                    </div>
                    <div>
                      <label className="block text-sm font-semibold text-ink mb-1">{t('GitHub URL')}</label>
                      <input value={editData.github_url || ''} onChange={(e) => setEditData({ ...editData, github_url: e.target.value })} className="w-full px-4 py-2.5 rounded-lg border border-line focus:border-primary focus:ring-2 focus:ring-primary/20 text-sm" placeholder={t('https://github.com/...')} />
                    </div>
                    <div>
                      <label className="block text-sm font-semibold text-ink mb-1">{t('Demo URL')}</label>
                      <input value={editData.demo_url || ''} onChange={(e) => setEditData({ ...editData, demo_url: e.target.value })} className="w-full px-4 py-2.5 rounded-lg border border-line focus:border-primary focus:ring-2 focus:ring-primary/20 text-sm" placeholder={t('https://...')} />
                    </div>
                    <div className="flex justify-end gap-3">
                      <Button variant="secondary" type="button" onClick={() => setEditing(false)}>{t('Cancel')}</Button>
                  ...
                  <div className="space-y-6">
                    <div>
                      <h3 className="font-semibold text-primary-navy mb-2">{t('Description')}</h3>
                      <p className="text-ink-soft whitespace-pre-wrap text-sm leading-relaxed">{solution.description}</p>
                    </div>
                    {solution.approach && (
                      <div>
                        <h3 className="font-semibold text-primary-navy mb-2">{t('Approach')}</h3>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm border-t border-line pt-4">
                      {solution.tech_stack?.length > 0 && (
                        <div className="col-span-2">
                          <p className="text-xs font-semibold uppercase tracking-wide text-ink-muted mb-1">{t('Tech Stack')}</p>
                      ...
                      {solution.estimated_timeline && (
                        <div>
                          <p className="text-xs font-semibold uppercase tracking-wide text-ink-muted mb-1">{t('Timeline')}</p>
                          <p className="font-medium text-primary-navy">{solution.estimated_timeline}</p>
                        </div>
                      )}
                      {solution.estimated_budget && (
                        <div>
                          <p className="text-xs font-semibold uppercase tracking-wide text-ink-muted mb-1">{t('Budget')}</p>
                    ...
                    <div className="flex gap-2 flex-wrap">
                      {solution.github_url && (
                        <a href={solution.github_url} target="_blank" rel="noopener noreferrer">
                          <Button variant="secondary" size="sm">{t('🐙 GitHub')}</Button>
                        </a>
                      )}
                      {solution.demo_url && (
                        <a href={solution.demo_url} target="_blank" rel="noopener noreferrer">
                          <Button size="sm">{t('🚀 Live Demo')}</Button>
                        </a>
                      )}
                    </div>
                  </div>
          ...
          <div className="space-y-4">
            {user?.role === 'industry' && (
              <Card>
                <h3 className="font-bold text-primary-navy mb-1">{t('Industry Actions')}</h3>
                <p className="text-xs text-ink-soft mb-4">{t('Partner with this university team to bring this solution to life.')}</p>
                ...
                {existingCollab ? (
                  <div className="space-y-3">
                    ...
                    <p className="text-xs text-ink-muted text-center">{t('Stage:')} <span className="font-semibold capitalize">{existingCollab.stage.replace('_', ' ')}</span></p>
                    <Link to={`/industry/collaborations/${existingCollab.id}`} className="block">
                      <Button className="w-full">{t('Open Workspace →')}</Button>
                    </Link>
                  </div>
                ) : (
                  <div className="space-y-3">
                    {INDUSTRY_ACTIONS.map((action) => (
                      ...
                      >
                        ...
                        <div className="text-left">
                          <div>{actionBusy === action.id ? t('Processing…') : action.label}</div>
                    ...
                    {!industry && (
                      <p className="text-xs text-ink-muted text-center mt-2">
                        {t('You need an industry profile to collaborate.')}{' '}
                        <Link to="/industry/dashboard" className="text-primary underline">{t('Create one →')}</Link>
                      </p>
                    )}
                  </div>
                )}
            ...
            {/* Metadata card */}
            <Card>
              <h3 className="font-bold text-primary-navy mb-3">{t('Details')}</h3>
              <dl className="space-y-2 text-sm">
                <div className="flex justify-between">
                  <dt className="text-ink-muted">{t('Solution ID')}</dt>
                  <dd className="font-mono text-xs text-ink-soft">{solution.id}</dd>
                </div>
                <div className="flex justify-between">
                  <dt className="text-ink-muted">{t('Submitted')}</dt>
